...
@interface WCSectionFooterView()<SDCycleScrollViewDelegate>
...
@implementation WCSectionFooterView
...
- (UIView *)refreshView {
    if (!_refreshView) {
        ...
        _refreshView.frame = CGRectMake(0, 0, ScreenW, ScreenW / 4);
        ...
        titleLabel.center = CGPointMake(ScreenW / 2, ScreenW / 8 - 10);
        [_refreshView addSubview:titleLabel];
        
        UIButton *refreshBtn = [[UIButton alloc] init];
        refreshBtn.bounds = (CGRect){CGPointZero, CGSizeMake(60, 20)};
        refreshBtn.center = CGPointMake(ScreenW / 2, ScreenW / 8 + 12);
...
- (SDCycleScrollView *)cycleScrollView {
    if (!_cycleScrollView) {
        _cycleScrollView = [SDCycleScrollView cycleScrollViewWithFrame:CGRectMake(0, 0, ScreenW, ScreenW / 4) delegate:self placeholderImage:[UIImage imageNamed:@"home_section_loading"]];
...
- (DotActivityIndicatorView *)indicatorView {
    if (!_indicatorView) {
        ...
        _indicatorView.center = CGPointMake(ScreenW / 2, ScreenW /  8);
...
- (void)refreshBtnClick {
    if ([self.delegate respondsToSelector:@selector(sectionFooterViewDidClickRefreshBtn:)]) {
        [self.delegate sectionFooterViewDidClickRefreshBtn:self];
    }
}

- (void)cycleScrollView:(SDCycleScrollView *)cycleScrollView didSelectItemAtIndex:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(sectionFooterView:selectedIndex:)]) {
        [self.delegate sectionFooterView:self selectedIndex:index];
    }
}
